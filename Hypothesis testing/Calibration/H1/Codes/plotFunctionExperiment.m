function plotFunctionExperiment(param, model, parIndex, param_opt, simTime_11mM, simTime_5p5mM,time_end)

%plotFunctionExperiment Plots simulated glucose and insulin responses
%against experimental data, and other mechanistic variables derived from
%the model
%   Input parameters: param: Set of acceptable parameters from optimization
%                     model: Model to simulate 
%                     parIndex: Index of parameters in the mode
%                     param_opt: Optimal parameter set from optimization
%                     simTime_11mM: Simulation times for hyperglycemia
%                     simTime_5p5mM: Simulation times for normoglycemia
%                     time_end: End time for simulations
...
k_ins_nM = 1/144;                   % Conversion factor for insulin
...
% Simulate for hyperglycemic concentration
...
for j = 1 : size(param,1) % For each row of the param set.
    ...
    % Calculate error to get the optimal
    G_measured_11=plotData.Gmeasured_11mM(j,ismembertol(simTime_11mM,EXPDATA.time{1}));
    I_measured_11=plotData.Imeasured_11mM(j,ismembertol(simTime_11mM,EXPDATA.time{2}));
    
    % Error hyperglycemia
    
    error(j,1)=sum(((transpose(G_measured_11)- EXPDATA.mean{1}).^2)./(EXPDATA.SD{1}).^2)+...
        sum(((transpose(I_measured_11)- EXPDATA.mean{2}).^2)./(EXPDATA.SD{2}).^2);
...
plotData.S_i_11mM_max(1:nTime_11mM) = max(plotData.S_i_11mM,[],1);
plotData.S_i_11mM_min(1:nTime_11mM) = min(plotData.S_i_11mM,[],1);
plotData.G_slow_11mM_max(1:nTime_11mM) = max(plotData.G_slow_11mM,[],1);
plotData.G_slow_11mM_min(1:nTime_11mM) = min(plotData.G_slow_11mM,[],1);
plotData.beta_vol_11mM_max(1:nTime_11mM) = max(plotData.beta_vol_11mM,[],1);
plotData.beta_vol_11mM_min(1:nTime_11mM) = min(plotData.beta_vol_11mM,[],1);
plotData.S_cap_11mM_max(1:nTime_11mM) = max(plotData.S_cap_11mM,[],1);
plotData.S_cap_11mM_min(1:nTime_11mM) = min(plotData.S_cap_11mM,[],1);
plotData.F_hyper_11mM_max(1:nTime_11mM) = max(plotData.F_hyper_11mM,[],1);
plotData.F_hyper_11mM_min(1:nTime_11mM) = min(plotData.F_hyper_11mM,[],1);
...
% Simulate for normoglycemic concentration
...
for j = 1 : size(param,1) % For each row of the param set.
    ...
    plotData.F_hyper_5p5mM(j,1:nTime_11mM) = simData_5p5mM{1}.variablevalues(:,29);
     ...
     % Calculate error to get the optimal
    G_measured_5p5=plotData.Gmeasured_5p5mM(j,ismembertol(simTime_11mM,EXPDATA.time{5}));
    I_measured_5p5=plotData.Imeasured_5p5mM(j,ismembertol(simTime_11mM,EXPDATA.time{6}));
    
    % Error normoglycemia
    
    error(j,2)=sum(((transpose(G_measured_5p5)- EXPDATA.mean{5}).^2)./(EXPDATA.SD{5}).^2)+...
        sum(((transpose(I_measured_5p5)- EXPDATA.mean{6}).^2)./(EXPDATA.SD{6}).^2);
    
        
end

error_sim=sum(error,2); %Error in the simulations (to find optimal value)
[cost_opt,index_opt]=min(error_sim);

% Calculate maximal and minimal values of the predictions

plotData.Gmeasured_5p5mM_max(1:nTime_11mM)=max(plotData.Gmeasured_5p5mM,[],1);
plotData.Gmeasured_5p5mM_min(1:nTime_11mM)=min(plotData.Gmeasured_5p5mM,[],1);
plotData.Imeasured_5p5mM_max(1:nTime_11mM)=max(plotData.Imeasured_5p5mM,[],1);
plotData.Imeasured_5p5mM_min(1:nTime_11mM)=min(plotData.Imeasured_5p5mM,[],1);

plotData.Gmeasured_5p5mM_max(1:nTime_11mM)=max(plotData.Gmeasured_5p5mM,[],1);
plotData.Gmeasured_5p5mM_min(1:nTime_11mM)=min(plotData.Gmeasured_5p5mM,[],1);
plotData.Imeasured_5p5mM_max(1:nTime_11mM)=max(plotData.Imeasured_5p5mM,[],1);
plotData.Imeasured_5p5mM_min(1:nTime_11mM)=min(plotData.Imeasured_5p5mM,[],1);
plotData.Gliver_5p5mM_max(1:nTime_11mM)=max(plotData.Gliver_5p5mM,[],1);
plotData.Gliver_5p5mM_min(1:nTime_11mM)=min(plotData.Gliver_5p5mM,[],1);
plotData.Gislets_5p5mM_max(1:nTime_11mM)=max(plotData.Gislets_5p5mM,[],1);
plotData.Gislets_5p5mM_min(1:nTime_11mM)=min(plotData.Gislets_5p5mM,[],1);
plotData.Iliver_5p5mM_max(1:nTime_11mM)=max(plotData.Iliver_5p5mM,[],1);
plotData.Iliver_5p5mM_min(1:nTime_11mM)=min(plotData.Iliver_5p5mM,[],1);
plotData.Iislets_5p5mM_max(1:nTime_11mM)=max(plotData.Iislets_5p5mM,[],1);
plotData.Iislets_5p5mM_min(1:nTime_11mM)=min(plotData.Iislets_5p5mM,[],1);
...
plotData.S_i_5p5mM_max(1:nTime_11mM) = max(plotData.S_i_5p5mM,[],1);
plotData.S_i_5p5mM_min(1:nTime_11mM) = min(plotData.S_i_5p5mM,[],1);
plotData.G_slow_5p5mM_max(1:nTime_11mM) = max(plotData.G_slow_5p5mM,[],1);
plotData.G_slow_5p5mM_min(1:nTime_11mM) = min(plotData.G_slow_5p5mM,[],1);
plotData.beta_vol_5p5mM_max(1:nTime_11mM) = max(plotData.beta_vol_5p5mM,[],1);
plotData.beta_vol_5p5mM_min(1:nTime_11mM) = min(plotData.beta_vol_5p5mM,[],1);
plotData.S_cap_5p5mM_max(1:nTime_11mM) = max(plotData.S_cap_5p5mM,[],1);
plotData.S_cap_5p5mM_min(1:nTime_11mM) = min(plotData.S_cap_5p5mM,[],1);
plotData.F_hyper_5p5mM_max(1:nTime_11mM) = max(plotData.F_hyper_5p5mM,[],1);
plotData.F_hyper_5p5mM_min(1:nTime_11mM) = min(plotData.F_hyper_5p5mM,[],1);

% Calculate best fits for both glycemic regimes

% Hyperglycemia
plotData.Gmeasured_11mM_opt = plotData.Gmeasured_11mM(index_opt,:);
plotData.Imeasured_11mM_opt = plotData.Imeasured_11mM(index_opt,:);
plotData.Gliver_11mM_opt = plotData.Gliver_11mM(index_opt,:);
plotData.Gislets_11mM_opt = plotData.Gislets_11mM(index_opt,:);
plotData.Iliver_11mM_opt = plotData.Iliver_11mM(index_opt,:);
plotData.Iislets_11mM_opt = plotData.Iislets_11mM(index_opt,:);
plotData.U_ii_11mM_opt = plotData.U_ii_11mM(index_opt,:);
plotData.U_id_11mM_opt = plotData.U_id_11mM(index_opt,:);

% Normoglycemia
plotData.Gmeasured_5p5mM_opt = plotData.Gmeasured_5p5mM(index_opt,:);
plotData.Imeasured_5p5mM_opt = plotData.Imeasured_5p5mM(index_opt,:);
plotData.Gliver_5p5mM_opt = plotData.Gliver_5p5mM(index_opt,:);
plotData.Gislets_5p5mM_opt = plotData.Gislets_5p5mM(index_opt,:);
plotData.Iliver_5p5mM_opt = plotData.Iliver_5p5mM(index_opt,:);
plotData.Iislets_5p5mM_opt = plotData.Iislets_5p5mM(index_opt,:);
plotData.U_ii_5p5mM_opt = plotData.U_ii_5p5mM(index_opt,:);
plotData.U_id_5p5mM_opt = plotData.U_id_5p5mM(index_opt,:);


%% Simulations fitted to experimental measurements

% Hyperglycemia
% Glucose and insulin in culture compartments

figure()
sgtitle('Simulated responses, hyperglycemia')
subplot(2,3,1)
...
hold on
plot(simTime_11mM,plotData.Gliver_11mM_opt,'Color', color11mM,'Linewidth',2);
hold off
...
figure()
sgtitle('Disease progression variables, hyperglycemia')
subplot (3,2,1)
...
xticks([0 48 96 144 192 240 288 336])
xlabel('Time (h)','FontSize',20);
ylabel('Islets volume (L)','FontSize',20)
set(gca,'TickDir','out','FontSize',20);
box off
...
%% Simulation fitted to experimental measurements
% Normoglycemia

% Glucose and insulin in culture compartments
figure()
sgtitle('Simulated responses, normoglycemia')
...
%% Disease progression variables
figure()
sgtitle('Disease progression variables, normoglycemia')
subplot(2,3,1)
...
area1=[simTime_11mM,fliplr(simTime_11mM)];
area2=[plotData.beta_vol_5p5mM_max,fliplr(plotData.beta_vol_5p5mM_min)];
f=fill(area1,area2,color5p5mM,'LineStyle','none');
alpha(f,.2)
...
area1=[simTime_11mM,fliplr(simTime_11mM)];
area2=[plotData.F_hyper_5p5mM_max*100,fliplr(plotData.F_hyper_5p5mM_min*100)];
f=fill(area1,area2,color5p5mM,'LineStyle','none');
alpha(f,.2)
...
hold on
plot(simTime_5p5mM,plotData.Gmeasured_11mM(index_opt,:),'Color', color11mM,'Linewidth',2);
...
errorbar(EXPDATA.time{5}, EXPDATA.mean{5}, EXPDATA.SD{5},'Color', color5p5mM,...
    'LineStyle', 'n', 'Marker',marker,'MarkerFaceColor',color5p5mM, 'MarkerSize',8)
hold on
plot(simTime_5p5mM,plotData.Gmeasured_5p5mM(index_opt,:),'Color', color5p5mM,'Linewidth',2);
