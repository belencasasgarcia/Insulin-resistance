...
EXPDATA.time{7} = G_hyper_lowHC_GTT([1 2 3 4],1);    % Time (absolute) (hours)
EXPDATA.mean{7} = G_hyper_lowHC_GTT([1 2 3 4],2);    % Time (absolute) (hours)
EXPDATA.SD{7} = G_hyper_lowHC_GTT([1 2 3 4],4);      % Time (absolute) (hours)

%SD_relation_G = EXPDATA.SD{7}./EXPDATA.mean{7}*100;

%EXPDATA.SD{7} = EXPDATA.mean{7}*0.05 + abs(11-EXPDATA.mean{7}(1));

% Look at the relation between SD and mean

% Insulin data from hyperglycemia, low HC
EXPDATA.time{8} = I_hyper_lowHC_GTT([1 2 3 4],1);    % Time (absolute) (hours)
EXPDATA.mean{8} = I_hyper_lowHC_GTT([1 2 3 4],2);    % Time (absolute) (hours)
EXPDATA.SD{8} = I_hyper_lowHC_GTT([1 2 3 4],4);      % Time (absolute) (hours)
...
EXPDATA.SD{9}./EXPDATA.mean{9}*100
