...
offset_11mM = 10.586 - 11;
offset_5p5mM = 11.82 - 11;
...
for j = 1 : size(param,1) % For each row of the param set.
    ...
    % Change offsets
    %param(j,parIndex.i_Sigma)=1.5*param(j,parIndex.i_Sigma);
    %param(j,parIndex.i_S_i)=param(j,parIndex.i_S_i)/2;

    %param(j,parIndex.i_Sigma)=1.6e+07;
    ...
    iC_11mM=[(11+param(j,parIndex.i_delta_G_1_11mM))*param(j,parIndex.i_V_m_hep) (11+param(j,parIndex.i_delta_G_1_11mM))*param(j,parIndex.i_V_m_islets)...
   ...
   param(j,parIndex.i_delta_I_13) = param(j,parIndex.i_delta_I_13_11mM);
    ...
    plotData.ratio_id_ii_11mM(j,1:nTime_11mM)=plotData.U_id_11mM(j,1:nTime_11mM)./plotData.U_ii_11mM(j,1:nTime_11mM);
...
% Ratio between insulin dependent and insulin independent 

figure()

area1=[simTime_11mM,fliplr(simTime_11mM)];
area2=[plotData.ratio_id_ii_11mM_max,fliplr(plotData.ratio_id_ii_11mM_min)];
f=fill(area1,area2,color11mM);
alpha(f,.2)
...
%title('Measured')
xlabel('Time (h)','FontSize',20);
ylabel('Ratio between id and ii glucose uptake','FontSize',20)
set(gca,'TickDir','out','FontSize',20);
box off
...
area1=[simTime_11mM,fliplr(simTime_11mM)];
area2=[plotData.int_G_5p5mM_max,fliplr(plotData.int_G_5p5mM_min)];
f=fill(area1,area2,color5p5mM);
alpha(f,.2)
...
%title('Measured')
xlabel('Time (h)','FontSize',20);
ylabel('Accumulated glucose','FontSize',20)
set(gca,'TickDir','out','FontSize',20);
box off
